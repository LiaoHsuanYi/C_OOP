...
void AppTest::SetUp()
{
    ...
    // p =new Polygon(arr, 4);
    // Polygon tmp = CreatePolygon(arr, 4);
    // p = &tmp;
...
TEST_F(AppTest, ADDTEST)
{
    ...
    ASSERT_NEAR(app.getIndex(0) -> area(), 314.159, 0.01);
...
TEST_F(AppTest, EXCHANGE_TEST)
{
    ...
    // exchange the memory space of the pointer


    // app.getIndex(0) -> getName();
    ASSERT_NEAR(app.getIndex(0) -> area(), 8, 0.1);
    ASSERT_NEAR(app.getIndex(1) -> area(), 314.159, 0.01);
    ASSERT_NEAR(app.getIndex(2) -> area(), 16, 0.1);
