...
/*MathVector::MathVector(const MathVector &input) : MathVector() // copy constructor, & = pass by reference (do not copy)
{
    std:: cout << "constructor" << std:: endl;
...
MathVector MathVector::operator+(const MathVector &input) const //operator overloading
// a = b + c; -> add(b, c);
{
    ...
    int j = 0; 
    for (itr = _arrV.begin(); i < _arrV.end(); itr++)
    {
        tmp[j] = (*itr) + input.getIndex(i);
        j += 1;
    }*/
    ...
    delete[] tmp;
    return m;
    // no copy constructor <- compiler copy elision
}
MathVector MathVector::operator-(const MathVector &input) const //operator overloading
{
    if(this -> _arr == nullptr || this -> _dim == 0)
...
/*std::ostream operator << () // TBD(To Be Determined)
{
